<template>
    <div>
        <div @click="openModal" tabindex="0" class="relative w-50 cursor-pointer">
            <FwbInput type="text" placeholder="Search...(Cmd+K)" readonly
        ...
        <FwbModal v-if="isOpen" @close="closeModal" size="4xl">
            ...
            <template #body>
                <ul v-if="results.length" class="divide-y divide-gray-200 text-sm">
                    <li v-for="result in results" :key="result.id" class="px-4 py-2 cursor-pointer hover:bg-gray-100">
                        {{ result.name }}
                    </li>
                </ul>
