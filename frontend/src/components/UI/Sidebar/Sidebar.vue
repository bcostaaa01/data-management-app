<template>
    <div class="relative shrink-0 h-full transition-[width] duration-300" :class="visible ? 'w-64' : 'w-16'">
        <aside
            class="h-full flex flex-col border-r dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-100 overflow-hidden"
            aria-label="Sidebar">
            <nav class="flex-1 overflow-y-auto overflow-x-hidden px-3 py-2">
                ...
                <div v-else class="flex flex-col items-center gap-1">
                    <router-link v-for="link in links" :key="link.to" :to="link.to" @click="visible = true"
        ...
        <button @click="toggleSidebar" :aria-label="visible ? t('sidebar.collapse') : t('sidebar.expand')"
            class="absolute top-4 -right-3 z-10 flex items-center justify-center w-6 h-6 rounded-full border bg-gray-50 dark:bg-gray-800 dark:border-gray-700 shadow-sm hover:bg-gray-100 hover:text-gray-900 dark:hover:bg-gray-700 dark:hover:text-white transition-colors duration-200">
            <FontAwesomeIcon :icon="visible ? faAngleDoubleLeft : faAngleDoubleRight" class="w-3 h-3" />
        </button>
    </div>
</template>
